...
**Create the tables and matices** <br>
In this part we make the tables and the matrices, only the comlums and rows, basically the information with which we work
...
#Matrix
TrowsAtt <- rbind(KobeBryant_FTA, JoeJohnson_FTA, LeBronJames_FTA, CarmeloAnthony_FTA, DwightHoward_FTA, ChrisBosh_FTA, ChrisPaul_FTA, KevinDurant_FTA, DerrickRose_FTA, DwayneWade_FTA)
...
colnames(TrowsAtt) <- Seasons
rownames(TrowsAtt) <- Players
#
```
<br><br>
**Hit 1 for the player playing style**
``` R
Tab1 <- (Points-FreeThrows)/FieldGoals
```
<br><br>
**Hit 2 fill the Blanks**
``` R
which(is.na(Tab1[9 , 8]))
Tab1[9 , 8] <- 0
```
<br><br>
...
```
<br><br>
**Rename the rows and check matrix**
```R
...
```R
myplot(TrowsAtt/Games)
```
<br><br>
**Part 2**

``` R
myplot(FreeThrows/TrowsAtt)
```
<br><br>
**Part 3** <br>
For this part is need to create the nex table and matrix, with your function 
``` R
